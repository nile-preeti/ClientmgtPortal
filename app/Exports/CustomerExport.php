...
class CustomerExport implements FromCollection, WithHeadings, WithStyles, WithEvents
{
    protected $users;
    protected $month;
    protected $year;
    ...
    public function collection()
    {
        ...
        foreach ($this->users as $user) {
            ...
            $services = \App\Models\JobSchedule::with(['service', 'subCategory', 'user']) // Include user to get employee name
            ...
            // Format services and concatenate name, category, employee name, and working hours
            $serviceDetails = [];
            foreach ($services as $service) {
                $hours = $this->getTotalWorkingHoursPerService($service->user_id, $service->id, $this->month, $this->year);
                $serviceDetails[] = 
                    ($service->service->name ?? 'N/A') . ' - ' .  // Service name
                    ($service->subCategory->sub_category ?? 'N/A') . ' - ' . // Sub category
                    ($service->user->name ?? 'N/A') . ' - ' . // Employee name
                    ($hours ? $hours . ' hrs' : 'N/A'); // Working hours
            }

            // Concatenate service details for each user in the same column
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                // Enable wrap text for the Services column (column D)
                $servicesRange = 'F2:F' . $event->sheet->getHighestRow();
