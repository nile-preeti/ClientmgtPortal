                        ...
                        <div class="hrmodule-punching-controls-box">
                            ...
                            <div class="hrmodule-punching-item-action">
                                <div class="punching-btn">
                                    ...
                                    <div class="info" id="checkinInfo">Not checked in</div>
                                </div>
                                <div class="punching-btn">
                                    <button id="checkoutBtn" class="checkoutBtn" disabled>Check-out</button>
                                    <div class="info" id="checkoutInfo">Not checked out</div>
                                </div>
                            </div>
                            <div class="hrmodule-punching-item-action">
                                <div class="punching-btn">
                                    <a href="{{ route('attendance_records') }}" style="text-decoration: underline">User
                                        Attendance</a>

                                </div>
                                <div class="punching-btn">
                                    <a href="{{ route('holidays') }}" style="text-decoration: underline">Holidays</a>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            if (!localStorage.getItem('user')) {
                ...
                // Show modal when needed (Example)
                myModal.show();
            }
            ...
            $("#loginForm").on("submit", async function(e) {
                ...
                if (result.status === "success") {
                    ...
                    window.location.reload(); // Reload the page to proceed
                } else {
            ...
            function initializeButtons(lat, lng) {
                ...
                checkinBtn.addEventListener("click", async () => {
                    ...
                    const result = await saveDataToPHP("{{ route('attendance.store') }}", {
                ...
                checkoutBtn.addEventListener("click", async () => {
                    ...
                    const result = await saveDataToPHP("{{ route('attendance.update') }}", {
                        _token: csrfToken, // Include CSRF token
                        user_id: user.id,
                ...
                //     const result = await saveDataToPHP("{{ route('attendance.fetch') }}" + "?id=" +
                //         user.id, {
                //             _token: csrfToken, // Include CSRF token
                //             userId: user.id,
                //             action: "fetch",
            ...
            function fetchRecords() {
                $.get("{{ route('attendance.fetch') }}" + "?id=" + user.id, function(data) {
                    if (data.success) {
                        if (data.records) {
