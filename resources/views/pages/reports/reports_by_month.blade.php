                        ...
                        <div class="iq-card-body">
                            <div class="search-filter-info">
                                ...
                                <div class="col-sm-12 col-md-2">
                                    ...
                                    <div class="col-sm-12 col-md-2">
                                        <div class="form-group">
                                            <a class="btnChangePassword"  href="{{ route('employee.reports') }}?export=true&month={{ request('month', '') }}&year={{ request('year', '') }}"><i
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        ...
                                                        @if($item->working_hours > 0)
                                                            <a href="javascript:void(0);"
                                                            data-user-id="{{ $item->id }}"
                                                            data-month="{{ request('month') ?? now()->month }}"
                                                            data-year="{{ request('year') ?? now()->year }}"
                                                            class="view-weekly-hours"
                                                            title="View Details">
                                                                <i class="fa fa-info-circle text-info ms-2"></i>
                                                            </a>
                                                        @endif
...
@push('js')
    ...
    $(document).on('click', '.view-service-details', function () {
        ...
        $.ajax({
            url: '{{ route("user.service.details") }}',
            type: 'GET',
            data: { user_id: userId,
                month: month,
                year: year },
...
document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('.view-weekly-hours').forEach(function (el) {
        el.addEventListener('click', function () {
            let userId = this.getAttribute('data-user-id');
            let month = this.getAttribute('data-month');
            let year = this.getAttribute('data-year');

            const url = `{{ url('/admin/weekly-working-hours') }}/${userId}?month=${month}&year=${year}`;
            ...
            fetch(url)
                ...
                .then(data => {
                    let html = '';
                    if (data.length === 0) {
                        html = `<p>No working hours found for this month.</p>`;
                    } else {
                        data.forEach((item, index) => {
                            html += `<p>Week ${index + 1}: ${item.toFixed(3)} hrs</p>`;
                        });
                    }
                    document.getElementById('weeklyHoursContent').innerHTML = html;
                    new bootstrap.Modal(document.getElementById('weeklyHoursModal')).show();
                })
                .catch(error => {
                    document.getElementById('weeklyHoursContent').innerHTML = `<p class="text-danger">Error loading data.</p>`;
